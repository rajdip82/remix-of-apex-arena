...
import { Home, Swords, Trophy, Users, User, Bell, ClipboardList, Search, Shield, LogOut } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen bg-background pb-24 md:pb-0">
      ...
      <aside className="fixed left-0 top-0 z-40 hidden h-screen w-64 flex-col border-r border-border bg-surface/60 backdrop-blur-xl md:flex">
        ...
        <nav className="flex-1 space-y-1 px-3">
          {nav.map((n) => {
            ...
            const showBadge = n.to === "/app/notifications" && unreadCount > 0;
            return (
              <Link key={n.to} to={n.to} className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "bg-primary/15 text-foreground" : "text-muted-foreground hover:bg-white/5 hover:text-foreground"}`}>
                ...
                <span className="relative">
                  ...
                  {showBadge && <span className="absolute -right-1.5 -top-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-primary text-[8px] font-bold text-white">{unreadCount > 9 ? "9+" : unreadCount}</span>}
                </span>
                {n.label}
              </Link>
            );
        ...
        <div className="m-3 space-y-2">
          {isAdmin && (
            <Link to="/admin" className="flex items-center gap-2 rounded-xl border border-accent/40 bg-accent/5 px-3 py-2.5 text-xs text-accent transition hover:border-accent hover:bg-accent/10">
              <Shield className="h-4 w-4" /> Admin Panel
            </Link>
          )}
          <button
            onClick={handleSignOut}
            className="flex w-full items-center gap-2 rounded-xl border border-border bg-black/30 px-3 py-2.5 text-xs text-muted-foreground transition hover:border-red-500/40 hover:text-red-400"
          >
      ...
      <header className="sticky top-0 z-30 hidden h-16 items-center justify-between border-b border-border bg-background/70 px-6 backdrop-blur-xl md:flex md:pl-72">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 text-sm text-muted-foreground w-72">
            ...
            <input className="w-full bg-transparent outline-none" placeholder="Search tournaments, players..." />
          </div>
          <Link to="/app/notifications" className="relative grid h-10 w-10 place-items-center rounded-xl border border-border hover:border-primary">
            <Bell className="h-4 w-4" />
            {unreadCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[9px] font-bold text-white">{unreadCount > 9 ? "9+" : unreadCount}</span>}
          </Link>
          <Link to="/app/profile" className="grid h-10 w-10 place-items-center rounded-xl gradient-primary text-xs font-bold">
            {profile?.avatar_url
      ...
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border bg-background/80 px-4 backdrop-blur-xl md:hidden">
        ...
        <Link to="/app/notifications" className="relative grid h-9 w-9 place-items-center rounded-lg border border-border">
          <Bell className="h-4 w-4" />
          {unreadCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[9px] font-bold text-white">{unreadCount > 9 ? "9+" : unreadCount}</span>}
        </Link>
      </header>
