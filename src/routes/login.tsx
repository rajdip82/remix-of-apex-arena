...
import { Swords, Phone, ArrowRight } from "lucide-react";
    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center bg-background p-6 sm:p-10">
        ...
        >
          ...
          <button className="flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-white text-black py-3 text-sm font-semibold transition hover:brightness-95">
          ...
          <form
            onSubmit={(e) => { e.preventDefault(); window.location.href = "/verify-otp"; }}
            className="space-y-3"
          >
            ...
            <div className="flex items-center gap-2 rounded-xl border border-border bg-black/30 px-3 focus-within:border-primary focus-within:ring-glow">
              ...
              <input required type="tel" placeholder="98765 43210" pattern="[0-9 ]{10,12}" className="flex-1 bg-transparent py-3 text-sm outline-none" />
            </div>
            <button className="flex w-full items-center justify-center gap-2 rounded-xl gradient-primary py-3 text-sm font-bold uppercase tracking-widest shadow-glow transition hover:brightness-110">
              Send OTP <ArrowRight className="h-4 w-4" />
            </button>
          </form>

          <p className="text-center text-sm text-muted-foreground">
            {mode === "login" ? <>New here? <Link to="/signup" className="font-semibold text-primary hover:underline">Create account</Link></> : <>Already have an account? <Link to="/login" className="font-semibold text-primary hover:underline">Sign in</Link></>}
          </p>
          <p className="text-center text-[10px] uppercase tracking-widest text-muted-foreground">By continuing you agree to our <Link to="/terms" className="text-foreground/70 hover:text-primary">Terms</Link></p>
        </motion.div>
      </div>
    </div>
  );
